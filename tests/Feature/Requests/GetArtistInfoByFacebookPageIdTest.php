...
test('can retrieve artist info by facebook id', function () {
    ...
    $facebookPageId = '67253243887';

    $response = $bandsInTownApi->send(new GetArtistInfoByFacebookPageId($facebookPageId));

    $mockClient->assertSent(GetArtistInfoByFacebookPageId::class);

    $mockClient->assertSent(function (Request $request, BandsInTownResponse $response) {
        return $request instanceof GetArtistInfoByFacebookPageId
